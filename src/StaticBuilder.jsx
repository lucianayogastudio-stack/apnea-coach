...
// ── Main Static Builder ───────────────────────────────────────────────────────
export default function StaticBuilder({ initialData, onSave, isClient: isClientProp , difficultyPicker=null}) {
    ...
    <div style={{fontFamily:"'DM Sans','Helvetica Neue',sans-serif",color:"#1a1a1a"}}>
      ...
      {exercises.map((ex,i)=>(
        <div key={ex.id}
          ...
          onDrop={!isClient?e=>{ e.preventDefault(); const from=parseInt(e.dataTransfer.getData("exIdx")); reorderExercises(from,i); }:undefined}
          style={{display:"flex",alignItems:"flex-start",gap:6,marginBottom:0}}>
          {!isClient && (
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,paddingTop:14,flexShrink:0}}>
              ...
              <button onClick={()=>reorderExercises(i,i-1)} disabled={i===0} title="Move up"
              ...
              <button onClick={()=>reorderExercises(i,i+1)} disabled={i===exercises.length-1} title="Move down"
